...
long initapp (long argc, char **argv)
{
	...
	kpzload("png/knumb.png",&numb[0].f,&numb[0].p,&numb[0].x,&numb[0].y);
	for(i=1;i<11;i++)
	{
	...
	kpzload("png/kasci9x12.png",&asci[32].f,&asci[32].p,&asci[32].x,&asci[32].y);
	for(i=33;i<128+18;i++)
	{
	...
	kpzload("png/target.png",&target.f,&target.p,&target.x,&target.y);
				...
				if (j > 1)
				{
					...
					if (typemessage[0] == '/')
					{
						...
						if (!_memicmp(&typemessage[1],"curvy=",6))
						{
							...
							{
								kv6data *tempkv6;
								for(tempkv6 = curvykv6; tempkv6=genmipkv6(tempkv6); ); //Generate all lower mip-maps here:
